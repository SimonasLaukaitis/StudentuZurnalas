...
bool Student::operator==(Student& c1) {
    if (c1.finalGrade == finalGrade && finalGrade == c1.finalGrade ) {
        return true;
    }
    else
        return false;
...
//bool Student::operator==(const Student& c1, const Student& c2)
//{
//    return (c1.finalGrade == c2.finalGrade);
//}
...
void gradesCorection() {
    ...
    for (Student a : objVector)
    {
        if (homeWorkCount < a.grades.size())
        {
            homeWorkCount = a.grades.size();
        }
        else
        {
            homeWorkCount = homeWorkCount;
    ...
    cout << "HOMEWORK COUNT : " << homeWorkCount << endl;
    int tempcount = 0;
    ...
    {   
        ...
        for (int j = 0; j < homeWorkCount; j++) {
            ...
            if (j >= tempcount) {
                ...
                cout << "nulio pridejimas" << endl;
                // KAZKODEL NEPUSHBACKINA
                a.grades.push_back(0);
            }
...
void bufer_nusk(std::string read_vardas) {
    ...
    nuskriaustukai = raskMinkstus(objVector);
   ...
   //////////////////////////////////////////////////////////////////////
    ...
    // kad nesididintu talpyklos kas kart nuskaicius
    nuskriaustukai.clear();
    objVector.clear();
...
list<Student> raskMinkstus(list<Student>& studentai) {

    list<Student> minksti;
    list<Student>::iterator it = studentai.begin();
...
bool gavoSkola(Student a) {
    if(a.finalGrade < 5.00) { return true; }
    else { return false; }
